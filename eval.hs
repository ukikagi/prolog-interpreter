...
answer_sub prog ((q:qs, sb) : res) =
  answer_sub prog (res ++ adding)
     where adding = 
             do Rule hd tl <- fresh_rule (vars_query (q:qs)) <$> prog
...
answer prog query =
  let vars = Set.unions (map vars_prop query) in
    nub $ map (\sb -> Map.filterWithKey (\k a -> Set.member k vars) sb) $
...
fresh_rule taboovars rule =
  let oldvars  = Set.toList $ vars_rule rule in
  let newtvars = take (length oldvars)
                     [TVar v | i <- [1..], let v = "X"++(show i), notElem v taboovars] in
    subst_rule (Map.fromList $ zip oldvars newtvars) rule
...
unify ((TVar x, t):constr) =
  if t == TVar x then unify constr
  else if appear x t then Nothing
  else let sb1 = Map.singleton x t in
       do sb2 <- unify (subst_constr sb1 constr)
